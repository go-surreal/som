...
func dbCreateNew[T any](ctx context.Context, db *dbConn, target string, data any) (*T, error) {
	statement := fmt.Sprintf("CREATE %s CONTENT $data", target)
...
func (c *dbConn) Live(ctx context.Context, statement string, vars map[string]any) (<-chan []byte, error) {
	...
	go func() {
		...
		if ctx.Err() != nil {
			...
			defer cancel()
			if err := surrealdb.Kill(killCtx, c.conn, liveID); err != nil {
