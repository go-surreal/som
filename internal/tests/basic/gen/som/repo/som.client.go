...
func (c *dbConn) Live(ctx context.Context, statement string, vars map[string]any) (<-chan []byte, error) {
	...
	go func() {
		...
		if ctx.Err() != nil {
			...
			// from emitting further notifications, then wait for the
			// forwarding goroutine to exit and drain any remaining
			// notifications before closing the channel ourselves.
			if _, err := surrealdb.Query[any](killCtx, c.conn,
