...
func SelectArrayDecodeWeatherKey(data []byte) ([][]model.WeatherKey, error) {
	var rawResult []internal.QueryResult[[]WeatherKey]
	if err := cbor.Unmarshal(data, &rawResult); err != nil {
		return nil, err
	}
	...
	for i, row := range rawResult[0].Result {
		inner := make([]model.WeatherKey, len(row))
		for j, v := range row {
			inner[j] = ToWeatherKey(v)
		}
		out[i] = inner
	}
	return out, nil
...
func SelectArrayDecodeWeatherKeyPtr(data []byte) ([][]*model.WeatherKey, error) {
	var rawResult []internal.QueryResult[[]*WeatherKey]
	if err := cbor.Unmarshal(data, &rawResult); err != nil {
		return nil, err
	}
