// Code generated by github.com/go-surreal/som, DO NOT EDIT.
package query

import (
	som "som.test/gen/som"
	conv "som.test/gen/som/conv"
	lib "som.test/gen/som/internal/lib"
	model "som.test/model"
)

// squareModelInfo holds the model-specific unmarshal functions for Square.
var squareModelInfo = modelInfo[model.Square]{
	Fields: conv.SquareFields,
	UnmarshalAll: func(data []byte) ([]*model.Square, error) {
		return unmarshalAll(data, conv.ToSquarePtr)
	},
	UnmarshalOne: func(data []byte) (*model.Square, error) {
		return unmarshalOne(data, conv.ToSquarePtr)
	},
	UnmarshalSearchAll: func(data []byte, clauses []lib.SearchClause) ([]lib.SearchResult[*model.Square], error) {
		return unmarshalSearchAll(data, clauses, conv.ToSquarePtr)
	},
}

var squareRangeFn = rangeFn[model.Square](func(q *lib.Query[model.Square], from som.RangeFrom, to som.RangeTo) string {
	expr := ":"
	if !from.IsOpen() {
		expr += q.AsVar(from.Value().(som.ULID))
	}
	if !from.IsOpen() && !from.IsInclusive() {
		expr += ">"
	}
	expr += ".."
	if !to.IsOpen() && to.IsInclusive() {
		expr += "="
	}
	if !to.IsOpen() {
		expr += q.AsVar(to.Value().(som.ULID))
	}
	return expr
})

// NewSquare creates a new query builder for Square models.
func NewSquare(db Database) Builder[model.Square] {
	q := lib.NewQuery[model.Square]("square")
	return Builder[model.Square]{builder[model.Square]{
		db:      db,
		info:    squareModelInfo,
		query:   q,
		rangeFn: squareRangeFn,
	}}
}
