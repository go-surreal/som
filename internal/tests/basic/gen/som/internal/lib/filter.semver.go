...
type SemVer[M any] struct {
	*Base[M, sombase.SemVer, *SemVer[M], *Slice[M, sombase.SemVer, *SemVer[M]]]
}

func NewSemVer[M any](key Key[M]) *SemVer[M] {
	return &SemVer[M]{
		Base: NewBase[M, sombase.SemVer, *SemVer[M], *Slice[M, sombase.SemVer, *SemVer[M]]](key),
	}
}

func (b *SemVer[M]) Compare(other sombase.SemVer) *Numeric[M, int] {
