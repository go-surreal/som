...
func (l *LineStringSF) MarshalCBOR() ([]byte, error) {
	...
	points := make([]*PointSF, n)
	for i := 0; i < n; i++ {
		xy := seq.GetXY(i)
		pt := geom.NewPoint(geom.Coordinates{XY: xy})
		p := PointSF(pt)
		points[i] = &p
	}
...
func (p *PolygonSF) MarshalCBOR() ([]byte, error) {
	...
	rings := make([]*LineStringSF, 1+numHoles)

	extLS := LineStringSF(extRing)
	rings[0] = &extLS

	for h := 0; h < numHoles; h++ {
		hole := poly.InteriorRingN(h)
		holeLS := LineStringSF(hole)
		rings[1+h] = &holeLS
	}
...
func (m *MultiLineStringSF) MarshalCBOR() ([]byte, error) {
	...
	lines := make([]*LineStringSF, numLines)
