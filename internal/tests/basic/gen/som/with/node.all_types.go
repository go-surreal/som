...
func AllTypesFetchBit(field string) uint64 {
	switch field {
	case "field_node":
		return allTypesFetchedFieldNode
	case "field_node_ptr":
		return allTypesFetchedFieldNodePtr
	default:
		return 0
	}
}

func AllTypesFetchFields(bits uint64) []string {
	var fields []string
	if bits&allTypesFetchedFieldNode != 0 {
		fields = append(fields, "field_node")
	}
	if bits&allTypesFetchedFieldNodePtr != 0 {
		fields = append(fields, "field_node_ptr")
	}
	return fields
}

func AllTypesSetFetched(m *model.AllTypes, bits uint64) {
