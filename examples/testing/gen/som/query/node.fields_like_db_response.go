...
import (
	...
	with "github.com/go-surreal/som/examples/testing/gen/som/with"
...
func (q nodeFieldsLikeDBResponse) Paginate(ctx context.Context, pageSize int, cursor string) (<-chan LiveResult[*model.FieldsLikeDBResponse], error) {
	req := q.query.BuildAsLive()
	resChan, err := q.db.Live(ctx, req.Statement, req.Variables)
	if err != nil {
		return nil, fmt.Errorf("could not query live records: %w", err)
...
func (q nodeFieldsLikeDBResponse) PaginateAsync(ctx context.Context, pageSize int, cursor string) *asyncResult[string] {
