...
func (q nodeUser) Paginate(ctx context.Context, pageSize int, cursor string) (<-chan LiveResult[*model.User], error) {
	req := q.query.BuildAsLive()
	resChan, err := q.db.Live(ctx, req.Statement, req.Variables)
	if err != nil {
		return nil, fmt.Errorf("could not query live records: %w", err)
...
func (q nodeUser) PaginateAsync(ctx context.Context, pageSize int, cursor string) *asyncResult[string] {
