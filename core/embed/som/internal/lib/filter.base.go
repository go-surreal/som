...
type Base[M, T any, F, EF field[M]] struct {
	Key[M]
	conv func(T) any
}

func NewBase[M, T any, F, EF field[M]](key Key[M]) *Base[M, T, F, EF] {
	return &Base[M, T, F, EF]{Key: key}
}

func NewBaseConv[M, T any, F, EF field[M]](key Key[M], conv func(T) any) *Base[M, T, F, EF] {
	return &Base[M, T, F, EF]{Key: key, conv: conv}
}

func (b *Base[M, T, F, EF]) Equal(val T) Filter[M] {
	if b.conv != nil {
...
func (b *Base[M, T, F, EF]) NotEqual(val T) Filter[M] {
	mapped := any(val)
...
func (b *Base[M, T, F, EF]) In(vals []T) Filter[M] {
	if b.conv != nil {
...
func (b *Base[M, T, F, EF]) NotIn(vals []T) Filter[M] {
	mapped := any(vals)
...
func (b *Base[M, T, F, EF]) notSet(op Operator, val any) Filter[M] {
	return filter[M](func(ctx *context, _ M) string {
...
func (b *Base[M, T, F, EF]) Truth() *Bool[M] {
...
func (b *Base[M, T, F, EF]) Zero(is bool) Filter[M] {
	op := OpExactlyEqual
...
type BasePtr[M, T any, F, EF field[M]] struct {
	*Base[M, T, F, EF]
	*Nillable[M]
}

func NewBasePtr[M, T any, F, EF field[M]](key Key[M]) *BasePtr[M, T, F, EF] {
	return &BasePtr[M, T, F, EF]{
		Base:     NewBase[M, T, F, EF](key),
		Nillable: NewNillable[M](key),
	}
}

func NewBasePtrConv[M, T any, F, EF field[M]](key Key[M], conv func(T) any) *BasePtr[M, T, F, EF] {
	return &BasePtr[M, T, F, EF]{
		Base:     NewBaseConv[M, T, F, EF](key, conv),
		Nillable: NewNillable[M](key),
	}
}
