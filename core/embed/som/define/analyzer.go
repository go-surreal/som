...
const (
	...
	// Class tokenizer segments text into tokens by detecting changes
	// (digit, letter, punctuation, blank) in the Unicode class of characters.
	// It creates a new token when the character class changes, distinguishing
	// between digits, letters, punctuation, and blanks. This allows for flexible
	...
	// Punct tokenizer generates tokens by breaking the text whenever a punctuation
	// character is encountered. It’s suitable for tokenizing sentences or breaking
	// text into smaller units based on punctuation marks.
	//
...
var (
	// Ascii filter is responsible for processing tokens by replacing or removing
	// diacritical marks (accents and special characters) from the text. It helps
	// standardize text by converting accented characters to their basic ASCII
	// equivalents, making it more suitable for various text analysis tasks.
	//
	// Docs: https://surrealdb.com/docs/surrealql/statements/define/analyzer#ascii
	Ascii = Filter{Name: "ascii"}

	// Lowercase filter converts tokens to lowercase, ensuring that text is
	// consistently in lowercase format. This is often used to make text
	// case-insensitive for search and analysis purposes.
	//
	// Docs: https://surrealdb.com/docs/surrealql/statements/define/analyzer#lowercase
	Lowercase = Filter{Name: "lowercase"}

	// Uppercase filter converts tokens to uppercase, ensuring text consistency
	// in uppercase format. It can be useful when case-insensitivity is required
	// for specific analysis or search operations.
	//
	// Docs: https://surrealdb.com/docs/surrealql/statements/define/analyzer#uppercase
	Uppercase = Filter{Name: "uppercase"}
)

// Edgengram filter is used to create tokens that represent prefixes of terms.
// It generates a sequence of tokens that gradually build up a term, which can be useful
// for autocomplete or searching based on partial words. It accepts two parameters min
// and max which define the minimum and maximum amount of characters in the prefix.
//
...
// Mapper is a filter designed to enable lemmatization within SurrealDB.
// Lemmatization is the process of reducing words to their base or dictionary form.
// The mapper mechanism allows users to specify a custom dictionary file that maps
// terms to their base forms. This dictionary file is then used by SDB’s analyzer
// to standardize terms as they are indexed, improving search consistency.
//
// This is particularly useful for handling irregular verbs and other terms that the
// default “snowball” filter cannot handle. Lemmatization files are easy to put together
// and to find online, making it possible to customize full-text search for smaller languages.
//
// This path parameter points to the file that contains the term mappings for lemmatization.
//
...
// Ngram filter is used to create a sequence of "n" tokens from a given sample of text or speech.
// These items can be syllables, letters, words or base pairs according to the application.
// It accepts two parameters min and max which indicates that
// you want to create n-grams starting from min to size of max.
//
...
// Snowball filter applies Snowball stemming to tokens, reducing
// them to their root form and converts the case to lowercase.
//
