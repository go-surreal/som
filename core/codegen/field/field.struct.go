...
func (f *Struct) selectDecode(ctx Context) jen.Code {
	...
	funcName := "SelectDecode" + f.element.NameGo()
	if f.source.Pointer() {
		funcName += "Ptr"
	}

	return jen.Qual(convPkg, funcName)
}

func (f *Struct) selectDistinctDecode(_ Context) jen.Code {
	return nil
}
...
func (f *Struct) fieldFieldFunc(ctx Context) jen.Code {
	return jen.Func().
		Params(jen.Id("n").Id(ctx.Table.NameGoLower()).Types(def.TypeModel)).
		Id(f.NameGo()).Params().
		Id(f.element.NameGoLower()).Types(def.TypeModel).
