...
func (f *Slice) filterDefine(ctx Context) jen.Code {
	...
	case *Byte:
		return jen.Id(f.NameGo()).Op("*").Qual(ctx.pkgLib(), "ByteSlice").Types(typeModel)
...
func (f *Slice) filterInit(ctx Context) (jen.Code, jen.Code) {
	...
	case *Node, *Edge:
		{
			...
			return jen.Qual(ctx.pkgLib(), "NewSliceMaker").Types(typeModel, element.typeGo(), elemFilter).
					Call(makeElemFilter),
				jen.Call(
	...
	case *Byte:
		return jen.Qual(ctx.pkgLib(), "NewByteSlice").Types(typeModel),
			jen.Call(
				jen.Qual(ctx.pkgLib(), "Field").Call(jen.Id("key"), jen.Lit(f.NameDatabase())),
			)

	case *Enum:
		return jen.Qual(ctx.pkgLib(), "NewSlice").Types(typeModel, jen.Qual(ctx.SourcePkg, element.model.NameGo())),
			jen.Call(
				jen.Qual(ctx.pkgLib(), "Field").Call(jen.Id("key"), jen.Lit(f.NameDatabase())),
				makeElemFilter,
			)
...
func (f *Slice) filterFunc(ctx Context) jen.Code {
	...
	case *Node:
		{
			return jen.Func().
				Params(jen.Id("n").Id(ctx.Table.NameGoLower()).Types(typeModel)).Id(f.NameGo()).
				Params(
				...
				Types(
					typeModel, jen.Qual(f.SourcePkg, element.table.NameGo()), jen.Id(element.table.NameGoLower()).Types(typeModel),
				).
				Block(
	...
	case *Edge:
		{
			...
			if tableEqual(ctx.Table, element.table.In.table) {
				return jen.Func().
					...
					Params(jen.Id(element.table.NameGoLower() + "In").Index(typeModel)).
					Block(
						jen.Return(
							jen.Id("new" + element.table.NameGo() + "In").Index(typeModel).
								Call(
			...
			if tableEqual(ctx.Table, element.table.Out.table) {
				return jen.Func().
					...
					Params(jen.Id(element.table.NameGoLower() + "Out").Index(typeModel)).
					Block(
						jen.Return(
							jen.Id("new" + element.table.NameGo() + "Out").Index(typeModel).
								Call(
	...
	//			Id(f.NameGo()).Params().
	//			Op("*").Qual(ctx.pkgLib(), "Slice").
	//			Types(typeModel, element.typeGo(), elemFilter).
	//			Block(
	//				jen.Return(
	//					jen.Qual(ctx.pkgLib(), "NewSlice").Types(typeModel, element.typeGo(), elemFilter).
	//						Call(
