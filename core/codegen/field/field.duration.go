...
func (f *Duration) selectArrayDecode(ctx Context) jen.Code {
	dt := jen.Qual(path.Join(ctx.TargetPkg, def.PkgTypes), "Duration")

	if f.source.Pointer() {
		return jen.Func().Params(jen.Id("data").Index().Byte()).Params(jen.Index().Index().Op("*").Qual("time", "Duration"), jen.Error()).Block(
			jen.Return(jen.Id("unmarshalSelectArrayConvert").Call(jen.Id("data"), jen.Func().Params(jen.Id("v").Op("*").Add(dt)).Op("*").Qual("time", "Duration").Block(
	...
	return jen.Func().Params(jen.Id("data").Index().Byte()).Params(jen.Index().Index().Qual("time", "Duration"), jen.Error()).Block(
		jen.Return(jen.Id("unmarshalSelectArrayConvert").Call(jen.Id("data"), jen.Func().Params(jen.Id("v").Add(dt)).Qual("time", "Duration").Block(
