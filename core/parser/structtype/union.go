...
func (h *UnionHandler) Match(t gotype.Type, ctx *parser.TypeContext) bool {
	return IsUnion(t, ctx.OutPkg)
}

func (h *UnionHandler) Handle(t gotype.Type, ctx *parser.TypeContext) error {
	...
	if open {
		...
		ctx.Output.Unions = append(ctx.Output.Unions, &parser.Union{Name: t.Name(), Open: true})
	...
	ctx.Output.Unions = append(ctx.Output.Unions, &parser.Union{Name: t.Name()})

	return nil
}

func (h *UnionHandler) Validate(ctx *parser.TypeContext) error {
