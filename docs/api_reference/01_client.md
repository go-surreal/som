...
When creating a client, SOM automatically verifies that the connected SurrealDB server meets the minimum required version (currently **3.1.3**). If the version check fails, `NewClient` returns a `som.ErrUnsupportedVersion` error:

```go
client, err := som.NewClient(ctx, config)
if err != nil {
    if errors.Is(err, som.ErrUnsupportedVersion) {
        log.Fatal("SurrealDB server version too old, please upgrade to 3.1.3+")
    }
    log.Fatal(err)
}
```
